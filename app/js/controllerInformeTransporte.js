...
angularRoutingApp.controller('controllerInformeTransporte', ['$scope', '$http',
    function ($scope, $http) {        
    ...
    $scope.pedidoSelecNota="";
        ...
        $scope.rendicionMontosArreglo = new Array();
          $scope.rendicionMontosArreglo =  [ 
                          {monto: 20000, nameMonto: "$ 20.000", cantidad: '0'},
                          {monto: 10000, nameMonto: "$ 10.000", cantidad: '0'},
                          {monto:  5000, nameMonto: "$  5.000", cantidad: '0'},
                          {monto:  2000, nameMonto: "$  2.000", cantidad: '0'},
                          {monto:  1000, nameMonto: "$  1.000", cantidad: '0'},
                          {monto:   500, nameMonto: "$    500", cantidad: '0'},
                          {monto:     1, nameMonto: " Monedas", cantidad: '0'}      
...
$scope.buscarListInforme = function (){
  ...
  $http({
     ...
     data:  $.param({desde: $scope.desde,
        ...
        $scope.listInformeTransp = [];
        $scope.listInformeTransp = data;
      
      
        $scope.listInformeTransp.forEach(c => {
            if(c.dataMonto!= ""){
            c.dataMonto = JSON.parse(c.dataMonto);
             }else{
                 c.dataMonto = "";
             }
            });
...
$scope.verDetalle = function($idTransp, $tranpObser, $idTranspPed, $selTransp){
    /*aqui**/
      $scope.rendicionMontos= [];
      $scope.cierre         = [];
      
      $scope.cierre = $selTransp.dataMonto;
          
    
    if($scope.cierre != ""){
        $scope.rendicionMontos =  $scope.cierre;
       }else{
           
         $scope.rendicionMontos=  $scope.rendicionMontosArreglo;
       }
...
$scope.getValorSumaMonto = function(){
    var total = 0;
    for(var i = 0; i < $scope.rendicionMontos.length; i++){
        var totalPed = $scope.rendicionMontos[i];

                total += parseInt(totalPed.monto * totalPed.cantidad);
...
$scope.guardarDetallePago = function(){
    	var arrayDeta=[];

        for(var i = 0; i < $scope.rendicionMontos.length; i++){
        var tMonto = $scope.rendicionMontos[i];   
			  var objDeta= new Object();           
                 objDeta.monto      = tMonto.monto;
                 objDeta.nameMonto  = tMonto.nameMonto;
                 objDeta.cantidad   = tMonto.cantidad;
                 objDeta.total      = tMonto.total;
              arrayDeta.push(objDeta);
    }
	
	const objDetalles= JSON.stringify(arrayDeta); 
    
    
    	$http({
     method : 'POST',
     url : 'FunctionIntranet.php?act=guardarDetallePago&tienda='+$scope.tiendaSeleccionada,
     data:  $.param({arrPed:objDetalles,
                     idInf:$scope.idInformeTransp}),
                headers : {'Content-Type': 'application/x-www-form-urlencoded'}  
  }).success(function(data){
        console.log(data);      
	 ...
	 var arrayProd    = [];
	       for(var i = 0; i < $scope.listInformeTranspDeta.length; i++){
                    var objDeta = new Object();
                      ...
                      objDeta.idInforme     = parseInt($scope.idInformeTransp); 
